...
class ModelConfig:
    ...
    save_path = 'bert_bilstm_kqa_3(2_epoch).pth'
    ...
    for e in range(config.epochs):
        ...
        for inputs, labels in data_train:
            ...
            if counter % config.print_every == 0:
                ...
                f = "bert_bilstm_kqa_3_loss(2_epoch).txt"
                a = "Epoch: {}/{}, ".format(e+1, config.epochs) + "Step: {}, ".format(counter) + "Loss: {:.6f}, ".format(loss.item()) + "Val Loss: {:.6f}".format(np.mean(val_losses))
...
def predict(test_comment_list, config):
    ...
    result_comments_id = tokenizer(result_comments,
                                    ...
                                    max_length=500,
                                    return_tensors='pt')
    # print(result_comments_id.keys())  # dict_keys(['input_ids', 'token_type_ids', 'attention_mask'])
...
if __name__ == '__main__':
    ...
    data = pd.read_csv('./Dynamic_Weight_Assignment_Model/train.csv',encoding='utf-8')
    result_comments = pretreatment(list(data['comment'].values))
    # print(result_comments[0])
