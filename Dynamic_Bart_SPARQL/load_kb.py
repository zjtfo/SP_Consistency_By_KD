...
class DataForSPARQL(object):
    def __init__(self, kb_path):
        ...
        # 替换名称中相邻的空格和制表符，这可能会在构建SPARQL查询时导致错误
        for con_id, con_info in self.concepts.items():
            con_info['name'] = ' '.join(con_info['name'].split())  # split()函数默认按空格分
        for ent_id, ent_info in self.entities.items():
            ent_info['name'] = ' '.join(ent_info['name'].split())

        # 获取所有属性键和谓词
        self.attribute_keys = set()  # 包含了属性中的key，还包括了属性中修饰符的key（即<qk>），关系中的修饰符的key（即<qk>）
        self.predicates = set()  # 包含了属性中的predicate
        self.key_type = {}  # 包含了属性的key_type，还包括了属性中修饰符的key_type，关系中修饰符的key_type
        for ent_id, ent_info in self.entities.items():
            for attr_info in ent_info['attributes']:  # 实体的属性
                self.attribute_keys.add(attr_info['key'])
                self.key_type[attr_info['key']] = attr_info['value']['type']
                for qk in attr_info['qualifiers']:  # 这里用到for循环是因为一个<qk>可能对应多个qualifiers（即修饰符条件）的值
                    self.attribute_keys.add(qk)
                    for qv in attr_info['qualifiers'][qk]:
                        self.key_type[qk] = qv['type']
        for ent_id, ent_info in self.entities.items():
            for rel_info in ent_info['relations']:  # 实体的关系
        ...
        # 注意：key_type是字符串/数量/日期之一，但date表示关键字可能具有Year类型的值
        self.key_type = { k:v if v!='year' else 'date' for k,v in self.key_type.items() }  # v不是'year'的就保持k:v，是'year'的就将v替换成'year'
    ...
    def _parse_value(self, value):
        if value['type'] == 'date':
            x = value['value']
            p1, p2 = x.find('/'), x.rfind('/')  # 这里使用/是因为date的格式是yyyy/mm/dd。这里的p1和p2分别是两次出现/的索引位置
