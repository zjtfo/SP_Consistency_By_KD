...
def train(args):
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_loader):
            ...
            # 生成的sparql
            outputs_s = model.module.generate(
                input_ids=y_ids,
                max_length = 500,
            )
            outputs_s = [tokenizer.decode(output_id, skip_special_tokens = True, clean_up_tokenization_spaces = True) for output_id in outputs_s]
            outputs_s = [post_process(output) for output in outputs_s]
            # print(outputs_s[0])
            # print(len(outputs_s))  # 16，就是等于batch_size大小
            
            
            # 生成的program
            outputs_p = teacher_model.module.generate(
                input_ids=source_ids,
                max_length = 500,
            )
            outputs_p = [teacher_tokenizer.decode(output_id, skip_special_tokens = True, clean_up_tokenization_spaces = True) for output_id in outputs_p]
            outputs_p = [post_process(output) for output in outputs_p]
            # print(outputs_p[0])
            # print(len(outputs_p))
            ...
            for i in range(len(outputs_s)):
                test_comments.append(str(outputs_p[i] + '[SEP]' + outputs_s[i].replace("\"", "'")))

            sim_count = classifier_predict(test_comments, net, net_tokenizer)
            
            print("sim_count->", sim_count)
            print("蒸馏权重->", sim_count / len(outputs_s))
            kd_weight = sim_count / len(outputs_s)
...
def main():
    ...
    # input and output
    parser.add_argument('--input_dir', default='./dynamic_bart_sparql/preprocessed_data')
    parser.add_argument('--output_dir', default='./dynamic_bart_sparql/output/checkpoint')

    parser.add_argument('--save_dir', default='./dynamic_bart_sparql/log', help='path to save checkpoints and logs')
