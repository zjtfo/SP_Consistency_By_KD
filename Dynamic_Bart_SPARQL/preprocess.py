...
def encode_dataset(dataset, vocab, tokenizer, test = False):
    ...
    print(max_seq_length)  # 304, 208, 87
    questions = []
    sparqls = []
    choices = []
    answers = []
...
def main():
    ...
    parser.add_argument('--output_dir', default='./dynamic_bart_sparql/preprocessed_data')
    ...
    print('Dump vocab to {}'.format(fn))  # Dump vocab to ../Bart_SPARQL/output/vocab.json
    with open(fn, 'w') as f:
        json.dump(vocab, f, indent=2)
    for k in vocab:
        print('{}:{}'.format(k, len(vocab[k])))  # answer_token_to_idx:81629
